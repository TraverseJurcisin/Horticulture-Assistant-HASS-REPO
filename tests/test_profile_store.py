...
@pytest.mark.asyncio
async def test_async_create_profile_handles_mapping_proxy_credentials(hass, tmp_path, monkeypatch) -> None:
    """Cloning from payloads with proxy credential mappings should succeed."""
    ...
    await store.async_create_profile(
        "Proxy Clone",
        clone_from={
            "display_name": "Source Proxy",
            "opb_credentials": types.MappingProxyType({"client_id": "proxy", "secret": "sec"}),
        },
    )

    clone = await store.async_get("Proxy Clone")
    assert clone is not None
    assert clone["opb_credentials"] == {"client_id": "proxy", "secret": "sec"}
