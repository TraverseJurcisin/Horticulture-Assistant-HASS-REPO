...
@pytest.mark.asyncio
async def test_async_create_profile_deduplicates_sequence_sensor_inputs(hass, tmp_path, monkeypatch) -> None:
    """Duplicate entries in sequence sensor bindings should be removed."""
    ...
    profile = await store.async_get("Duplicate Sensors")
    assert profile is not None
    assert profile["sensors"] == {"moisture": ["sensor.one", "sensor.two"]}
    general = profile["general"] if isinstance(profile.get("general"), dict) else {}
    assert general.get("sensors") == {"moisture": ["sensor.one", "sensor.two"]}
