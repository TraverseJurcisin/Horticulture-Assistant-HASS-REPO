...
def test_get_numeric_state_handles_unicode_minus():
    hass = types.SimpleNamespace(
        states=types.SimpleNamespace(get=lambda entity_id: types.SimpleNamespace(state="−5.5", attributes={}))
    )

    value = state_helpers.get_numeric_state(hass, "sensor.negative")
    assert value == -5.5
