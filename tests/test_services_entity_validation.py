...
@pytest.mark.asyncio
async def test_replace_sensor_accepts_ec_measurement(monkeypatch):
    hass = types.SimpleNamespace(
        states=DummyStates(
            {
                "sensor.ec": DummyState(
    ...
    call = types.SimpleNamespace(
        data={"profile_id": "plant", "measurement": "ec", "entity_id": "sensor.ec"}
    )

    await handler(call)

    registry.async_replace_sensor.assert_awaited_once_with("plant", "ec", "sensor.ec")
