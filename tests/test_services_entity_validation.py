...
@pytest.mark.asyncio
async def test_replace_sensor_accepts_enum_device_class(monkeypatch):
    hass = types.SimpleNamespace(
        states=DummyStates(
            {
                "sensor.temp": DummyState(device_class="temperature", unit_of_measurement="°C"),
    ...
    class DummyEntityRegistry:
        def __init__(self) -> None:
            self._entry = types.SimpleNamespace(
                device_class=services_module.SensorDeviceClass.TEMPERATURE,
        ...
        def async_get_entity_id(self, *_args, **_kwargs):  # pragma: no cover - unused in test
            return None

    entity_registry = types.SimpleNamespace(async_get=lambda _hass: DummyEntityRegistry())
    monkeypatch.setattr(services_module, "er", entity_registry)
    monkeypatch.setattr(services_module, "cv", types.SimpleNamespace(entity_id=lambda value: value))
