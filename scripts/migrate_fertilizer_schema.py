...
def build_composition_v6(product: Dict[str, Any]) -> Dict[str, Any]:
    ...
    micros: Dict[str, Any] = {}
    for source_key, target_key in (("Fe", "Fe"), ("Mn", "Mn"), ("Zn", "Zn"), ("Cu", "Cu"), ("B", "B"), ("Mo", "Mo"), ("Cl", "Cl"), ("Ni", "Ni")):
        value = nutrients.get(source_key)
        if value is not None:
...
def transform_record(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if isinstance(schema_block, dict) and schema_block.get("id") == SCHEMA_META["id"] and schema_block.get("version") == SCHEMA_META["version"]:
        return payload
...
def main() -> None:
    ...
    print(
        f"Updated {detail_count} fertilizer detail records and built index with {index_count} entries"
    )
