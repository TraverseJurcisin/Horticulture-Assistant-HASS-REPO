...
def _dataset(data):
    """Return dataset contents regardless of loader type."""
...
def list_supported_plants() -> list[str]:
    ...
    return list_dataset_entries(_dataset(_THRESHOLDS))
...
def get_monitoring_interval(plant_type: str, stage: str | None = None) -> int | None:
    ...
    return _get_interval(_dataset(_MONITOR_INTERVALS), plant_type, stage)
...
def risk_adjusted_monitor_interval(
    ...
    modifiers = _dataset(_RISK_MODIFIERS)
...
def next_monitor_date(
    ...
    return _next_date(_dataset(_MONITOR_INTERVALS), plant_type, stage, last_date)
...
def generate_monitoring_schedule(
    ...
    return _generate_schedule(_dataset(_MONITOR_INTERVALS), plant_type, stage, start, events)
...
def get_severity_action(level: str) -> str:
    ...
    actions = _dataset(_SEVERITY_ACTIONS)
...
def get_scouting_method(pest: str) -> str:
    ...
    methods = _dataset(_SCOUTING_METHODS)
...
def get_severity_thresholds(pest: str) -> Dict[str, float]:
    ...
    thresholds = _dataset(_SEVERITY_THRESHOLDS)
...
def estimate_pest_risk(
    ...
    factors = _dataset(_RISK_FACTORS).get(normalize_key(plant_type), {})
    if not factors:
        return {}
