...
def validate_profile_structure(plant_id: str, base_path: str = None, verbose: bool = False):
    ...
    expected_keys = {
        ...
        "microbiome.json": {
            "microbial_associations",
            "pathogenic_suppressors",
            "root_exudates",
        ...
        "reproductive.json": {
            ...
            "harvest_readiness",
            "self_pruning",
            "flower_to_fruit_rate",
        },
        "phenology.json": {
            "flowering_period",
            "fruiting_period",
            "dormancy_triggers",
            "stage_by_zone_estimates",
            "chill_hour_needs",
        },
        "genetics.json": {
            "gmo_status",
        ...
        "cultivar_lineage.json": {
            "parentage",
            "named_crosses",
            "hybridization_purpose",
            "ancestral_traits",
            "naming_origin",
            "divergence_from_wild",
            "related_commercial_varieties",
        },
        "regulatory.json": {
            "propagation_laws",
            "local_restrictions",
        ...
        "export_profile.json": {
            ...
            "fumigation_status",
            "customs_codes",
            "known_trade_restrictions",
            "preferred_international_markets",
        ...
        "processing.json": {
            ...
            "residue_breakdown",
            "transformation_compounds",
            "value_added_processing_options",
            "food_grade_standards",
            "pharmaceutical_standards",
        },
        "harvest.json": {
            "harvest_timing",
            "indicators_of_ripeness",
            "harvesting_method",
            "postharvest_storage",
            "spoilage_rate",
            "market_channels",
        ...
        "companion_plants.json": {
            "companion_species",
            "antagonists",
            "spatial_proximity_notes",
            "beneficial_biochemical_interactions",
            "known_competitive_exclusion_zones",
        },
        "rotation_guidance.json": {
            "rotation_families",
            "soil_impact",
            "antagonistic_residue_timing",
            "ideal_rotation_years",
            "soil_remediation_value",
            "successional_compatibility",
        },
        "biochemistry.json": {
            "flavor_aromatic_compounds",
            "medicinal_uses",
            "bioavailability",
            "nutrient_density",
            "antioxidant_load",
        ...
        "metabolite_profile.json": {
            "cannabinoids",
            "alkaloids",
            "terpenes",
            "polyphenols",
            "enzymatic_inhibitors",
            "hormone_influencers",
            "biosynthetic_pathway_markers",
        },
        "climate_adaptability.json": {
            "high_temp_tolerance",
            "low_temp_tolerance",
            "photoperiod_sensitivity",
            "rainfall_tolerance",
            "radiation_tolerance",
            "heat_unit_accumulation",
            "overwintering_behavior",
        },
        "zone_suitability.json": {
            "USDA_zones",
            "Köppen_climates",
            "optimal_latitude_band",
            "global_elevation_band",
            "subtropical_to_temperate_gradient",
            "known_zone_failures",
        },
    }
            ...
            if file_name in expected_nested_keys:
                for parent_key, subkeys in expected_nested_keys[file_name].items():
                    if parent_key in content and isinstance(content[parent_key], dict):
                        for subkey in subkeys:
                            if subkey not in content[parent_key]:
                                ...
                                _LOGGER.info(
                                    "File %s missing nested key '%s' in '%s'.",
                                    file_path,
                                    subkey,
                                    parent_key,
                                )
                    if parent_key in content and not isinstance(content[parent_key], dict):
                        _LOGGER.warning(
                            "In file %s, expected %s to be a dict, found %s.",
                            file_path,
                            parent_key,
                            type(content[parent_key]).__name__,
                        )
