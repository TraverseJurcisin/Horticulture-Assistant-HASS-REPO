...
class InventoryLot:
    ...
    storage_temp: float | None  # recommended storage temperature (°C)
...
class InventoryTracker:
    ...
    def add_inventory(
        ...
        expiration_date: datetime | None,
        price: float,
        derived_ingredients: dict[str, float],
    ...
    def log_usage(
        ...
        lots = sorted(
            self.inventory[product_name],
            key=lambda x: (x.expiration_date or datetime.max, x.manufacture_date),
        )
    ...
    def get_total_quantity(self, product_name: str, unit: str | None = None) -> float:
