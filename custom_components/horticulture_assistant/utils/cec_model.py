...
def _write_cec_record(output_path: str, plant_id: Optional[str], result_data: dict):
...
def log_measured_cec(plant_id: str, cec_value: float, include_warnings: bool = False) -> Optional[dict]:
...
def estimate_cec_from_media(plant_id: str, moisture_retention: float, ec_behavior: float, dryback_rate: float, include_warnings: bool = False) -> Optional[dict]:
    ...
    if not media_type or media_type not in MEDIA_CEC_ESTIMATES:
        _LOGGER.error("Unknown media type '%s' inferred for plant %s; CEC estimation not available.", media_type, plant_id)
        return None
