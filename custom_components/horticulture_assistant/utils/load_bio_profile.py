"""Helper for loading multi-file BioProfile fragments from disk.

Each profile has its own directory under ``plants/`` containing JSON fragments
describing the BioProfile. This module aggregates those fragments into a single
...
__all__ = ["LoadedBioProfile", "load_bio_profile", "load_plant_profile", "clear_profile_cache"]


@dataclass(slots=True)
class LoadedBioProfile:
    """Loaded BioProfile fragments assembled from disk."""
    ...
    def as_dict(self) -> dict[str, Any]:
        ...
        return {"plant_id": self.plant_id, "profile_id": self.plant_id, "profile_data": self.profile_data}

    @property
    def profile_id(self) -> str:
        """Return the BioProfile identifier (alias of :attr:`plant_id`)."""
...
def load_bio_profile(
    ...
    *,
    ...
    ----------
    plant_id: str
        Identifier for the profile (historically referred to as ``plant_id``).
    base_path: str | None, optional
        Base directory of BioProfiles, default ``plants/`` in the current
        working directory.
    ...
    """
    # Determine the base directory and profile directory
    ...
    if not plant_dir.is_dir():
        _LOGGER.error("BioProfile directory not found: %s", plant_dir)
        return {}
    ...
    try:
        ...
        _LOGGER.error("Error loading profile for '%s': %s", plant_id, e)
        return {}

    # If no profile sections were loaded, log an error and return empty
    if count_loaded == 0:
        _LOGGER.error(
            "No profile data loaded for '%s' (no valid profile files found).",
            plant_id,
        )
        return {}

    # Log summary of modules loaded
    _LOGGER.info("Loaded %d profile modules for '%s'.", count_loaded, plant_id)

    return LoadedBioProfile(plant_id=str(plant_id), profile_data=profile_data)


def clear_profile_cache() -> None:
    """Clear cached profile results from :func:`load_bio_profile`."""

    load_bio_profile.cache_clear()


# Backwards compatibility alias for older imports
load_plant_profile = load_bio_profile
