...
def generate_climate_profiles(plant_id: str, base_path: str = None, overwrite: bool = False) -> str:
    ...
    This function scaffolds two JSON files (`climate_adaptability.json` and
    `zone_suitability.json`) in the `plants/<plant_id>/` directory. It
    populates these files with predefined keys related to the plant's climate
    adaptability and zone suitability information, with all values defaulting to
    null (JSON null, represented as None in Python).
